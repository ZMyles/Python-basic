...
def center_window(self, w, h):
    screen_width = self.master.winfo_screenwidth() #gets users window width
    screen_height = self.master.winfo_screenheight() #gets users window hight

    # calculate x and y coord's to paint the app centered on the user's screen
...
def ask_quit(self):
    if messagebox.askokcancel("Exist program", "Okay to exit application"):
        # This closes app
        self.master.destroy()
        os._exit(0)
"""
...
def create_db(self):
    ...
    with conn:
        ...
        # commiting to save changes  and close database connection
        conn.commit()
    conn.close()
    first_run(self)

def first_run(self):
    data = ('Zavier', 'Myles', 'Zavier Myles', '111-111-1111', 'zmyles@gmail.com')
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
